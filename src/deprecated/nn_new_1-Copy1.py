...
class NNMODEL:
    ...
    def _eval_mse(self,dataloader):
        """
        Compute dataset-wide mean MSE in eval() mode.
        Returns a scalar float (average per-sample squared error over the whole loader).
        """
    ...
    def fit(self,Xtrain,Xvalid,ytrain,yvalid):
        ...
        for epoch in range(self.epochs):
            ...
            for Xbatch,ybatch in trainloader:
                ...
                ybatch = ybatch.to(self.device)
                optimizer.zero_grad()
                batchloss   = self.criterion(self.model(Xbatch).squeeze(),ybatch.squeeze())
                batchloss.backward()
                optimizer.step()
                trainloss += batchloss.item()*Xbatch.size(0)
            ...
            with torch.no_grad():
                for Xbatch,ybatch in validloader:
                    ...
                    ybatch = ybatch.to(self.device)
                    batchloss  = self.criterion(self.model(Xbatch).squeeze(),ybatch.squeeze())
                    validloss += batchloss.item()*Xbatch.size(0)
...
# self.model.train()
# sum_error_over_epoch = 0.0     # sum of per-sample squared errors across ALL batches
# num_samples_seen     = 0       # total number of samples we processed
...
#     meanbatchloss = self.criterion(ybatchpred.squeeze(),ybatch.squeeze())  # mean MSE for this batch
#     meanbatchloss.backward()
#     optimizer.step()
#     # Convert batch mean to batch total so larger batches count more
#     sum_error_over_epoch += float(meanbatchloss)*batchsize
#     num_samples_seen     += batchsize

# # Overall (sample-weighted) average MSE for the epoch
# trainloss = sum_error_over_epoch / num_samples_seen if num_samples_seen else float("nan")
...
#     break
           ...
           # eval MSE on training set

            # Eval mse on validation set

            wandb.log({
                'Epoch':epoch+1,
                'Training Loss':trainloss,
                'Validation Loss':validloss,
                'MSE on Training Set':evaltrain,
                'MSE on Validation Set':evalvalid,
