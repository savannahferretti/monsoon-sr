...
class NNMODEL:
    ...
    def __init__(self,inputsize,batchsize=64000,epochs=30,criterion=torch.nn.L1Loss(),learningrate=0.0001,patience=3):
        ...
        - criterion (torch.nn.Module): the loss function (defaults to L1)
...
def load(inputvars,filename,filedir=FILEDIR):
    ...
    with h5py.File(filepath,'r') as f:
        ...
        ytrain = torch.tensor(f['pr_train'][:],dtype=torch.float32).squeeze()
        yvalid = torch.tensor(f['pr_valid'][:],dtype=torch.float32).squeeze()
        ytest  = torch.tensor(f['pr_test'][:],dtype=torch.float32).squeeze()
        ...
        #     yvalid = (yvalid - mean) / std
        #     ytest  = (ytest  - mean) / std
        #     norm_params = {'mean': mean, 'std': std, 'log1p': LOG1P_TARGET}
        # else:
        #     norm_params = None
    ...
    for config in configs:
        ...
        wandb.init(
            ...
            config={
                ...
                'Optimizer':'Adam',
                'Loss Function':'Mean Absolute Error (L1)',
...
if __name__=='__main__':
    try:
        ...
        with open(f'{SAVEDIR}/nn_subset_results.pkl', 'wb') as f:
