...
class NNMODEL:
    ...
    def __init__(self,inputsize,batchsize=32000,epochs=30,criterion=torch.nn.MSELoss(),learningrate=0.0001,patience=3):
        '''
        ...
        self.model        = torch.nn.Sequential(
            torch.nn.Linear(self.inputsize,256),torch.nn.GELU(),
    ...
    def fit(self,Xtrain,Xvalid,ytrain,yvalid,*,exp_name=None,normparams=None,save_all_epochs=True):
        '''
        Purpose: Train a NN model and record per-epoch diagnostics.
        '''
    ...
    #     return total/len(dataloader.dataset)
            ...
            eval_train_mse = self._eval_mse(trainloader)
            eval_valid_mse = self._eval_mse(validloader)

            # -------- record history ----------
            self.history['epoch'].append(epoch)
            self.history['train_loss_inloop'].append(trainloss)
            self.history['valid_loss_inloop'].append(validloss)
            self.history['eval_train_mse'].append(eval_train_mse)
            self.history['eval_valid_mse'].append(eval_valid_mse)
            ...
            try:
                wandb.log({
                    'Epoch':epoch,
                    'Training Loss (in-loop)':trainloss,
                    'Validation Loss (in-loop)':validloss,
                    'Eval Train MSE (eval path)':eval_train_mse,
                    'Eval Valid MSE (eval path)':eval_valid_mse,
            ...
            if save_all_epochs:
                ckpt_path = self._save_checkpoint(epoch, optimizer, exp_name)
    ...
    for config in configs:
        ...
        wandb.init(
            ...
            config={
                ...
                'Network Architecture':'256→128→64→32→1 (GELU with no BatchNorm)',
                'Optimizer':'Adam',
