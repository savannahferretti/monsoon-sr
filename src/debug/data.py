...
def make_split(ntime,lats,lons,seed,startdate):
    ...
    latg = lat[None,:,None]
    long = lon[None,None,:]
    xbase  = (0.8*np.sin(2*np.pi*tt/30.0)+0.1*(latg/max(1e-6,np.max(np.abs(lat))))+0.05*(long/max(1e-6,np.max(np.abs(lon))))).astype(np.float32)
    xnoise = rng.normal(0.0,XNOISESTD,size=xbase.shape).astype(np.float32)
    x = (xbase+xnoise).astype(np.float32)
    ybase  = (3.0*x+1.5+AQUAD*(x**2)+ASIN*np.sin(2.0*np.pi*x)).astype(np.float32)
    ynoise = rng.normal(0.0,YNOISESTD,size=ybase.shape).astype(np.float32)
    y = (ybase+ynoise).astype(np.float32)
    # y = (3.0*x+1.5).astype(np.float32) # Old code only used htis for "y" (no noise, no nonlinearity)
    ds = xr.Dataset(
        data_vars={
            'x':(('time','lat','lon'),x,{'long_name':'x','units':'arb'}),
            'y':(('time','lat','lon'),y,{'long_name':'y','units':'arb'})},
