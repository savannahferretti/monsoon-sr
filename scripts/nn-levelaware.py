...
def add_positional_encoding(array):
    '''
    Purpose: Add altitude-based positional encoding to profile variables, handling NaN values properly.
    Args:
    - array (numpy.ndarray): input data with shape (nsamples, nlevels)
    Returns:
    - numpy.ndarray: array with proper positional encoding added
    '''
    if array.shape[1]==1:
        return array
    batchsize,nlevels = array.shape
    positions = np.linspace(1.0,0.0,nlevels)
    positionmatrix = np.tile(positions,(batchsize,1))
    positionmatrix = np.where(np.isnan(array),np.nan,positionmatrix)
    enhancedarray  = np.concatenate([array,positionmatrix],axis=1)
    return enhancedarray
    
def load(inputvars,filename,filedir=FILEDIR):
    '''
    Purpose: Load NN data splits from an HDF5 file for specific input variables with positional encoding.
    Args:
    ...
    with h5py.File(filepath, 'r') as f:
        ...
        for inputvar in inputvars:
            ...
            Xtrainenhanced = add_positional_encoding(Xtrainarray)
            Xvalidenhanced = add_positional_encoding(Xvalidarray)
            Xtestenhanced  = add_positional_encoding(Xtestarray)
            Xtraindata.append(Xtrainenhanced)
            Xvaliddata.append(Xvalidenhanced)
            Xtestdata.append(Xtestenhanced)
...
if __name__=='__main__':
    try:
        ...
        with open(f'{SAVEDIR}/nn_level_aware_subset_results.pkl', 'wb') as f:
