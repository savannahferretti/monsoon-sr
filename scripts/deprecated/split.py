...
def normalize(array, mean=None, std=None):
    '''
    Purpose: Normalize an array to zero mean and unit variance.
             If 2D (nsamples, nfeatures), do it **per feature (column-wise)**.
    Args:
    - array (np.ndarray): input array to normalize
    - mean (float or np.ndarray, optional): mean(s) for normalization
    - std  (float or np.ndarray, optional): std(s)  for normalization
    Returns:
    - (np.ndarray, mean, std): normalized array and the parameters used
    '''
    eps = 1e-8
